  ...
  if (trigger_type === "keyword") {
    ...
    if (!keywords || keywords.length === 0) {
      issues.push({
        ...
        message: "Disparos por palavra-chave precisam de ao menos uma palavra-chave.",
      });
    } else {
      ...
      if (blanks > 0) {
        issues.push({
          ...
          scope: "trigger",
          field: "trigger_config.keywords",
          message: `${blanks} palavra${blanks === 1 ? "-chave está" : "s-chave estão"} em branco — não vão corresponder a nada.`,
        });
      }
    }
  }
    ...
    case "collect_input": {
      ...
      if (!cfg.var_key?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: "O nó de coleta de resposta precisa de uma var_key para armazenar a resposta.",
        });
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: `var_key "${cfg.var_key}" deve conter apenas letras, números e underscore, e começar com uma letra ou underscore.`,
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó de coleta de resposta deve apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
    ...
    case "condition": {
      ...
      if (!cfg.subject_key?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "subject_key",
          message: "A condição precisa de um subject_key (nome da variável, id da tag ou nome do campo).",
        });
      }
      if (
        !cfg.operator ||
