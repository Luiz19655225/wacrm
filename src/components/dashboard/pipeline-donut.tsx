  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">Valor em negociações</h2>
        <p className="mt-0.5 text-xs text-muted-foreground">
          Negociações abertas por estágio
        </p>
      </header>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={GitBranch}
            title="Ainda não há negociações abertas"
            hint="Crie negociações em Negociações para ver o detalhamento por estágio aqui."
          />
        ) : (
          <>
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {s.dealCount} negociaç{s.dealCount === 1 ? 'ão' : 'ões'}
                  </span>
...
function Donut({ data, currency }: { data: PipelineDonutData; currency: string }) {
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Valor das negociações por estágio">
        {/* background ring */}
