    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Contagem de toda negociação neste funil que não está marcada como Perdida. Negociações ganhas continuam incluídas."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label="Valor do funil"
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip="Soma dos valores de todas as negociações deste funil, exceto as marcadas como Perdidas."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label="Ticket médio"
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip="Valor do funil dividido pelo total de negociações — o valor médio de uma negociação não perdida."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor ponderado"
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip="Receita esperada: o valor de cada negociação aberta × a probabilidade do seu estágio. Primeiro estágio ≈ 10%, os estágios avançam até 90%, Ganho = 100%. Negociações perdidas são excluídas."
        />
        <Metric
          icon={<Trophy className="h-4 w-4 text-primary" />}
          label="Ganhas este mês"
          value={String(stats.wonThisMonth)}
          tooltip="Negociações marcadas como Ganhas desde o primeiro dia do mês atual."
        />
        <Metric
          icon={<XCircle className="h-4 w-4 text-red-400" />}
          label="Perdidas este mês"
          value={String(stats.lostThisMonth)}
          tooltip="Negociações marcadas como Perdidas desde o primeiro dia do mês atual."
        />
      </div>
    </TooltipProvider>
  );
