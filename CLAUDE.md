...
Implementada localmente, **ainda sem commit/push/deploy** (aguardando revisão).
...
Pendências da Fase 2 (ver seção acima): revalidar o ajuste de `nextDueDate` contra o Asaas de produção antes do go-live, e confirmação explícita da revisão visual do painel pelo usuário.
